...
judge="${1:?usage: start_judge_tenacity_parallel.sh JUDGE_ALIAS [TRACE_LABEL|TRACE_PATH]}"
...
if [[ -z "${NVIDIA_API_KEY:-}" ]]; then
  ...
  echo "NVIDIA_API_KEY is not exported in this Terminal session" > "runs/nvidia-grades/${judge}-tenacity-status.log"
  exit 1
fi
...
if [[ ! -f "$traces" ]]; then
  mkdir -p runs/nvidia-grades
  echo "Trace file not found: ${traces}" > "runs/nvidia-grades/${trace_label}.${judge}-tenacity-status.log"
  exit 1
fi
...
request_timeout="${BFB_JUDGE_REQUEST_TIMEOUT:-180}"
num_retries="${BFB_JUDGE_NUM_RETRIES:-0}"
hard_timeout="${BFB_JUDGE_HARD_TIMEOUT:-240}"
retry_attempts="${TENACITY_RETRY_ATTEMPTS:-3}"
retry_initial="${TENACITY_RETRY_INITIAL_SECONDS:-10}"
retry_max="${TENACITY_RETRY_MAX_SECONDS:-120}"

output="${OUTPUT:-runs/nvidia-grades/${trace_label}.grades.${judge}.jsonl}"
log="${LOG:-runs/nvidia-grades/${trace_label}.${judge}-tenacity.log}"
status_log="${STATUS_LOG:-runs/nvidia-grades/${trace_label}.${judge}-tenacity-status.log}"
...
while IFS= read -r pid; do
  [[ -n "$pid" ]] && live+=("$pid")
done < <(pgrep -f "run_nvidia_grader.*--judge ${judge}.*${output}" || true)
...
pid=$!
echo "$(date '+%F %T') ${judge} Tenacity grader PID ${pid}" > "$status_log"
echo "trace_label=${trace_label}" >> "$status_log"
echo "traces=${traces}" >> "$status_log"
echo "output=${output}" >> "$status_log"
echo "sample_n=${sample_n} sample_seed=${sample_seed}" >> "$status_log"
echo "concurrency=${concurrency} provider_cap=${provider_cap}" >> "$status_log"
echo "timeouts request=${request_timeout} hard=${hard_timeout}" >> "$status_log"
echo "tenacity attempts=${retry_attempts} initial=${retry_initial} max=${retry_max}" >> "$status_log"
